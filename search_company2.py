...
def search_company():
	...
	for i in range(len(all_company)):
		...
		annualhigh = all_company[i][codeDataName.index("연중최고")]
		annuallow = all_company[i][codeDataName.index("연중최저")]
		price = all_company[i][codeDataName.index("현재가")]

		price = abs(price) # 가격하락 시 마이너스 값을 가지는 경우가 있음.
		annuallow = abs(annuallow)
		percent = str(round((price/annualhigh) * 100)) + "%"
		lowpcnt = str(round((price/annuallow) * 100) - 100) + "%"
...
#			continue
		...
		row = [code, name, stsum, per, eps, roe, pbr] + \
			...
			[qp_revenue, qp_profit, multiple, str(annualhigh), str(annuallow), str(price), percent, lowpcnt]
		rows.append(row)
